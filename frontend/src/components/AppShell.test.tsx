...
import { getActiveClinicId, setSessionUser } from "../api/client";
import { AppShell, navigationForRole } from "./AppShell";

function mockShellFetch(clinics = [{ id: 1, name: "Demo klinika", timezone: "Europe/Zagreb" }]) {
...
describe("navigacija prema zadatku i ulozi", () => {
  ...
  test("topbar jasno prikazuje ulogu običnim hrvatskim nazivom", () => {
    renderShell("demo_nurse");
    expect(screen.getByText("Uloga:")).toBeTruthy();
    expect(screen.getByText("Medicinska sestra/tehničar")).toBeTruthy();
  });
