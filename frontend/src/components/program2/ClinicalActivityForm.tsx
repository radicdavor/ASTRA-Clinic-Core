...
function StructuredField({ field, value, disabled, onChange }: { field: ClinicalFormField; value: unknown; disabled: boolean; onChange: (value: unknown) => void }) {
  ...
  return <fieldset className="structured-repeatable-field" disabled={disabled}><legend>{field.label}{field.required && " *"}</legend>
...
function FormField({ field, value, disabled, onChange }: { field: ClinicalFormField; value: unknown; disabled: boolean; onChange: (value: unknown) => void }) {
  if (clinicalFormStructuredTypes.has(field.type)) return <StructuredField field={field} value={value} disabled={disabled} onChange={onChange}/>;
...
export function ClinicalActivityForm({ journeyId, activity, serviceName, onChanged }: { journeyId: number; activity: JourneyActivity; serviceName: string; onChanged: () => Promise<void> }) {
  ...
  useEffect(() => { setForm(null); setData({}); setError(""); setConfirmation(null); }, [activity.id]);
  async function run(action: () => Promise<void>) { setBusy(true); setError(""); try { await action(); } catch (reason) { setError(reason instanceof Error ? reason.message : "Radnja nije spremljena."); } finally { setBusy(false); } }
  ...
  async function save() { await run(async () => setForm(await api<ClinicalFormInstance>(`${base}/form`, { method: "PATCH", body: JSON.stringify({ data }) }))); }
  async function completeForm() { await run(async () => setForm(await api<ClinicalFormInstance>(`${base}/form/complete`, { method: "POST" }))); }
  ...
  return <section className="journey-panel clinical-activity-form">
    ...
    {form && <><div className="clinical-form-meta"><span>Verzija {form.form_version.version}</span><span>{formStatusLabels[form.status] ?? "Nepoznato stanje"}</span></div>{form.form_version.sections_json.map(section => <fieldset key={section.section_key} disabled={busy || locked}><legend>{section.title ?? "Klinički nalaz"}</legend><div className="clinical-form-grid">{section.fields.map(field => <FormField key={field.field_key} field={field} value={data[field.field_key]} disabled={busy || locked} onChange={value => setData(current => ({ ...current, [field.field_key]: value }))}/>)}</div></fieldset>)}<div className="clinical-form-actions">{!locked && <button type="button" disabled={busy} onClick={save}><Save size={15}/>Spremi skicu</button>}{primary && <button type="button" className="primary" disabled={busy || primary.disabled} onClick={primary.action}>{primary.icon}{primary.label}</button>}</div></>}
