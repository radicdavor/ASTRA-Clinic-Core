...
export function ClinicalActivityForm({ journeyId, activity, serviceName, onChanged }: { journeyId: number; activity: JourneyActivity; serviceName: string; onChanged: () => Promise<void> }) {
  ...
  return <section className="journey-panel clinical-activity-form">
    <header><div><span className="eyebrow">Aktivnost {activity.sequence}</span><h2>{serviceName}</h2><p>{new Date(activity.planned_start).toLocaleTimeString("hr-HR", { hour: "2-digit", minute: "2-digit" })}–{new Date(activity.planned_end).toLocaleTimeString("hr-HR", { hour: "2-digit", minute: "2-digit" })}</p></div><span className={`activity-status ${activity.status}`}>{activity.status === "in_progress" ? "U tijeku" : activity.status === "completed" ? "Završeno" : "Planirano"}</span></header>
