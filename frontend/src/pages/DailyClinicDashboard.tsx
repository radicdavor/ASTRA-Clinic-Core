...
function operationalState(row: DashboardRow): OperationalState {
  ...
  if (["ready_for_clinician", "in_encounter"].includes(row.workflow_stage)) { const mayOpen = row.allowed_actions.includes("open_encounter"); return { tone: "active", label: row.workflow_stage === "in_encounter" ? "Pregled u tijeku" : "Čeka pregled/pretragu", detail: row.workflow_stage === "in_encounter" ? "Klinički susret je otvoren." : "Prijem je završen; pacijent čeka liječnika ili pretragu.", action: mayOpen ? "encounter" : "open", actionLabel: mayOpen ? (row.workflow_stage === "in_encounter" ? "Nastavi pregled" : "Otvori pregled") : "Otvori", icon: <Stethoscope size={15}/> }; }
  if (["arrived", "check_in_review"].includes(row.workflow_stage)) { const mayOpen = row.allowed_actions.includes("open_check_in"); return { tone: "active", label: "Stigao", detail: "Otvorite prijem: opći podaci, potpis i kratka pitanja prije pregleda.", action: mayOpen ? "reception" : "open", actionLabel: mayOpen ? "Otvori prijem" : "Otvori", icon: <ClipboardCheck size={15}/> }; }
...
export function DailyClinicDashboard() {
  ...
  return <section className="page clinic-day-page">
    ...
    <div className="clinic-day-filter-bar"><label className="clinic-day-search"><Search size={16}/><input aria-label="Pretraži pacijenta" placeholder="Pretraži pacijenta" value={query} onChange={event => setQuery(event.target.value)}/></label>{board.data.can_filter_clinician && <select aria-label="Liječnik" value={clinician} onChange={event => { setClinician(event.target.value); setClinic(""); setRoom(""); }}><option value="">Svi liječnici</option>{providers.data.filter(item => item.staff_role === "physician").map(item => <option key={item.id} value={item.id}>{item.full_name}</option>)}</select>}<select aria-label="Problem" value={blocker} onChange={event => setBlocker(event.target.value)}><option value="">Sva stanja</option><option value="true">S problemom</option><option value="false">Bez problema</option></select><details className="clinic-day-advanced"><summary><Filter size={15}/>Dodatni filtri{advancedFilterCount > 0 && <b aria-label={`${advancedFilterCount} aktivna dodatna filtra`}>{advancedFilterCount}</b>}</summary><div className="clinic-day-filters">{board.data.available_clinics.length > 1 && <select aria-label="Klinika" value={clinic} onChange={event => { setClinic(event.target.value); setRoom(""); }}><option value="">Sve klinike</option>{board.data.available_clinics.map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select>}<select aria-label="Prostorija" value={room} onChange={event => setRoom(event.target.value)}><option value="">Sve prostorije</option>{rooms.data.filter(item => !clinic || String(item.clinic_id ?? "") === clinic).map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select><select aria-label="Usluga" value={service} onChange={event => setService(event.target.value)}><option value="">Sve usluge</option>{services.data.map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select><select aria-label="Faza" value={stage} onChange={event => setStage(event.target.value)}><option value="">Sve faze</option><option value="ready_for_arrival">Naručen</option><option value="check_in_review">Stigao</option><option value="ready_for_clinician">Čeka pregled/pretragu</option><option value="in_encounter">Pregled u tijeku</option><option value="awaiting_payment">Čeka naplatu</option></select></div></details>{[clinician, clinic, room, service, stage, blocker, query].some(Boolean) && <button type="button" className="clear-filters" onClick={clearFilters}><X size={15}/>Očisti filtre</button>}</div>
    ...
    <ReceptionFloatingModal journeyId={receptionJourneyId} open={receptionJourneyId !== null} onClose={() => setReceptionJourneyId(null)} onCompleted={() => { setReceptionJourneyId(null); setRefresh(value => value + 1); }} />
  </section>;
}
