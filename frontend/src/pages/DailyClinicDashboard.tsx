...
export function DailyClinicDashboard() {
  ...
  return <section className="page clinic-day-page">
    ...
    <div className="clinic-day-table-wrap"><table className="clinic-day-table clinic-day-table-simple"><thead><tr><th>Vrijeme i pacijent</th><th>Usluga i liječnik</th><th>Trenutačno stanje</th><th>Sljedeća radnja</th></tr></thead><tbody>
      {board.data.rows.map(row => { const state = operationalState(row); const focus = focusFor(state); return <tr key={row.journey_id} className={`${state.tone}-row ${state.tone === "problem" ? "has-blocker" : ""}`}><td><span className="patient-time">{row.time.slice(0,5)}</span><Link to={`/journeys/${row.journey_id}?focus=${focus}`}>{row.patient_name}</Link><small>{board.data.available_clinics.length > 1 && row.clinic_name ? `${row.clinic_name} · ` : ""}{row.room_name}</small></td><td><strong>{row.service_name}</strong><small>{row.clinician_name}</small></td><td><div className="operational-state"><StatusSignal tone={state.tone} description={`${state.label}. ${state.detail}`}/><span><strong>{state.label}</strong><small>{state.detail}</small>{row.blockers.length > 1 && <small className="additional-problems">+{row.blockers.length - 1} dodatna problema</small>}</span></div></td><td className="clinic-day-actions">{state.action && <button type="button" onClick={() => runAction(row, state)}>{state.icon}{state.actionLabel}</button>}</td></tr>; })}
