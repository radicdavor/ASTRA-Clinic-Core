...
export function PatientJourneyWorkspace() {
  ...
  if (!journey.data) return <section className="page"><p>Učitavanje putovanja pacijenta...</p></section>;
  ...
  return <section className="page journey-workspace">
    <header className="journey-workspace-header"><div><span>PUTOVANJE PACIJENTA · #{j.id}</span><h1>{j.patient.first_name} {j.patient.last_name}</h1><p>Rođen/a: {j.patient.date_of_birth ?? "nije upisano"} · termin {j.appointment.date} u {j.appointment.start_time.slice(0, 5)}</p></div><div><strong>{j.current_stage}</strong><Link to={`/appointments/${j.appointment_id}`}>Termin #{j.appointment_id}</Link></div></header>
