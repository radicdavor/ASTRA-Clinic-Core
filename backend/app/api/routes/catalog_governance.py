...
from app.schemas.catalog_governance import FormBindingCreate, FormDefinitionCreate, FormVersionDraftCreate, PackageCreate, PackageMaterializeRequest, PackageVersionCreate
...
from app.services.catalog_governance import clone_form_version, create_binding, create_form_definition, create_package_version, materialize_package, publish_form_version, publish_package_version

router = APIRouter(prefix="/api", tags=["catalog-governance"])


@router.get("/service-packages")
def list_packages(db: Session = Depends(get_db), actor: Actor = Depends(require_permission("services.read"))):
    ...
    return [{"id": item.id, "package_key": item.package_key, "name": item.name, "description": item.description, "specialty_key": item.specialty_key, "versions": [{"id": version.id, "version": version.version, "status": version.status, "items": [{"id": row.id, "service_id": row.service_id, "activity_key": row.activity_key, "sequence": row.sequence, "duration_minutes": row.default_duration_minutes} for row in version.items]} for version in item.versions]} for item in packages]


@router.post("/service-packages", status_code=201)
def add_package(payload: PackageCreate, request: Request, db: Session = Depends(get_db), actor: Actor = Depends(require_permission("services.write"))):
...
def add_package_version(package_id: int, payload: PackageVersionCreate, request: Request, db: Session = Depends(get_db), actor: Actor = Depends(require_permission("services.write"))):
...
def publish_package(version_id: int, request: Request, db: Session = Depends(get_db), actor: Actor = Depends(require_permission("services.write"))):
...
def materialize(journey_id: int, version_id: int, payload: PackageMaterializeRequest, request: Request, db: Session = Depends(get_db), actor: Actor = Depends(require_permission("appointments.write"))):
