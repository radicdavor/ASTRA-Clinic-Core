...
def get_journey(db,id):
 item=db.scalar(select(PatientJourney).options(joinedload(PatientJourney.appointment),selectinload(PatientJourney.blockers),selectinload(PatientJourney.activities)).where(PatientJourney.id==id))
...
def update_encounter(journey_id:int,payload:EncounterUpdate,request:Request,db:Session=Depends(get_db),actor:Actor=Depends(require_permission("encounter.write"))):
 deny_activity_legacy_write(get_journey(db,journey_id),request,db,actor)
...
def complete_encounter(journey_id:int,request:Request,db:Session=Depends(get_db),actor:Actor=Depends(require_permission("encounter.complete"))):
 journey=get_journey(db,journey_id);deny_activity_legacy_write(journey,request,db,actor);item=get_encounter(db,journey_id)
