# Program 1 Phase J8 - Demo Known Limitations Pack

- no production use
- no real patient data
- no EMR certification
- no medical-device certification
- no automated diagnosis or treatment
- no findings/open-question/review/timeline write workflows
- no patient messaging
- no Task engine
- no Outcome Evidence
- no review approval workflow
- limited usability review
- limited legal/compliance review
- limited security review
- backup/restore not production-drilled
- audit export not production-approved

These limitations must be presented before stakeholder interpretation.
