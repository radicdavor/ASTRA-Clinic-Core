...
| `Clinical Readiness Snapshot` | `ClinicalReadinessSnapshot`, `clinical_readiness_snapshots`, migrations `0014`/`0015`, `clinical_readiness_snapshots.py` service, `POST /api/appointments/{appointment_id}/clinical-readiness-snapshots`, `GET /api/appointments/{appointment_id}/clinical-readiness-snapshots`, `GET /api/appointments/{appointment_id}/clinical-readiness-snapshots/{snapshot_id}`, `POST /api/appointments/{appointment_id}/clinical-readiness-snapshots/{snapshot_id}/supersede`, `AppointmentDetail` history/detail/capture/supersession surfaces | Backend capture/history/detail/supersession plus frontend history/capture/detail/supersession prototype; closed for demo/pilot guardrail use | B13 dodaje DB shape za immutable copied preview payload, reason, disclaimer, template metadata i future supersession fields. B14 dodaje interni capture service s audit eventom. B15 dodaje permission-gated backend capture endpoint. B16 dodaje read-only, appointment-scoped, summary-only history endpoint. B17 dodaje Appointment Workspace read-only history surface. B18 dodaje reason-required capture modal koji osvjezava history. B19 dodaje read-only detail endpoint i panel za full copied payload. B20 dodaje idempotency persistence i duplicate-capture guard. B21 hardena canonical disclaimer i immutable payload discipline. B22 dokumentira future additive supersession contract. B23 dodaje interni supersession service i audit event bez endpointa/UI-ja. B24 dodaje permission-gated supersession endpoint. B25 dodaje reason-required supersession UI u snapshot detail panelu. B26 stabilizira governance label-e i preview-only wording. B27 dodaje end-to-end i permission matrix regression coverage. B28 zatvara snapshot podfazu closure reportom i go/no-go matricom. | Preporuceni sljedeci korak je Snapshot Production Risk Hardening; real data, production i clinical enforcement ostaju no-go. |
...
| `AppointmentDetail` | Appointment workspace, materials, invoice, audit, read-only clinical readiness preview, snapshot history/detail and reason-required snapshot capture modal. | `Appointment`, `Material Consumption`, `Inventory Movement`, `Clinical Readiness Preview`, `Clinical Readiness Snapshot` | Aligned with caution | Preview/history/detail/capture nisu enforcement i ne smiju postati task, override, template editor, approval ili clearance surface. |
...
| --- | --- | --- | --- | --- |
| Clinical Readiness Gate not fully implemented | `Clinical Readiness Gate`, `Clinical Readiness Status`, `Clinical Readiness Item` | B3 read-only preview plus B4 static demo/pilot templates, B5 metadata transparency, B6 demo explicit binding, B7 demo version metadata, B8 snapshot non-implementation metadata, B9 snapshot persistence design docs, B10 migration review, B11 capture endpoint design, B12 permission/audit contracts, B13 persistence table/model, B14 internal capture service, B15 backend capture endpoint, B16 read-only history endpoint, B17 read-only history UI, B18 reason-required capture UI, B19 read-only detail view, B20 duplicate-capture guard, B21 canonical disclaimer/immutability hardening, B22 supersession contract docs, B23 internal supersession service, B24 supersession endpoint and B25 supersession UI | Confusion with `/api/readiness`; false clinical safety signal; premature blockers without override governance; static keyword/name/code config, demo version metadata and snapshot metadata could be mistaken for production rules; capture/history/detail/supersession UI could be mistaken for clinical approval if labels are poor. | Snapshot governance and safety label stabilization before enforcement |
