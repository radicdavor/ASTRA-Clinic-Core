...
| `Clinical Readiness Snapshot` | `ClinicalReadinessSnapshot`, `clinical_readiness_snapshots`, migration `0014_clinical_readiness_snapshots.py`, `clinical_readiness_snapshots.py` service, `POST /api/appointments/{appointment_id}/clinical-readiness-snapshots`, `GET /api/appointments/{appointment_id}/clinical-readiness-snapshots` | Backend capture/history prototype | B13 dodaje DB shape za immutable copied preview payload, reason, disclaimer, template metadata i future supersession fields. B14 dodaje interni capture service s audit eventom. B15 dodaje permission-gated backend capture endpoint. B16 dodaje read-only, appointment-scoped, summary-only history endpoint. Nema frontend UI-ja ili capture buttona. | Snapshot History UI Read-Only Surface prije capture buttona. |
...
| --- | --- | --- | --- | --- |
| Clinical Readiness Gate not fully implemented | `Clinical Readiness Gate`, `Clinical Readiness Status`, `Clinical Readiness Item` | B3 read-only preview plus B4 static demo/pilot templates, B5 metadata transparency, B6 demo explicit binding, B7 demo version metadata, B8 snapshot non-implementation metadata, B9 snapshot persistence design docs, B10 migration review, B11 capture endpoint design, B12 permission/audit contracts, B13 persistence table/model, B14 internal capture service, B15 backend capture endpoint and B16 read-only history endpoint | Confusion with `/api/readiness`; false clinical safety signal; premature blockers without override governance; static keyword/name/code config, demo version metadata and snapshot metadata could be mistaken for production rules; capture/history UI could be mistaken for clinical approval if future labels are poor. | Snapshot History UI Read-Only Surface before capture button, editor or enforcement |
