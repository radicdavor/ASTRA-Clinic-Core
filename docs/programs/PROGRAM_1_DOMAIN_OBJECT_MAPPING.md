...
| `Clinical Readiness Snapshot` | `ClinicalReadinessSnapshot`, `clinical_readiness_snapshots`, migration `0014_clinical_readiness_snapshots.py`, `clinical_readiness_snapshots.py` service | Internal service prototype | B13 dodaje DB shape za immutable copied preview payload, reason, disclaimer, template metadata i future supersession fields. B14 dodaje interni capture service s audit eventom, ali bez endpointa, UI-ja, route-level permission enforcementa ili history prikaza. | Snapshot Capture Endpoint Prototype prije UI capture buttona. |
...
| --- | --- | --- | --- | --- |
| Clinical Readiness Gate not fully implemented | `Clinical Readiness Gate`, `Clinical Readiness Status`, `Clinical Readiness Item` | B3 read-only preview plus B4 static demo/pilot templates, B5 metadata transparency, B6 demo explicit binding, B7 demo version metadata, B8 snapshot non-implementation metadata, B9 snapshot persistence design docs, B10 migration review, B11 capture endpoint design, B12 permission/audit contracts, B13 persistence table/model and B14 internal capture service | Confusion with `/api/readiness`; false clinical safety signal; premature blockers without override governance; static keyword/name/code config, demo version metadata and snapshot metadata could be mistaken for production rules; future capture could be mistaken for clinical approval if route permission/reason rules are not implemented. | Snapshot Capture Endpoint Prototype before UI capture button, editor or enforcement |
