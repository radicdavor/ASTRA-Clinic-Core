...
| `Patient Clinical Knowledge` | `/patients/{id}/clinical-summary`, `PatientKnowledgeItem`, PatientDetail cards, `patient_knowledge.py` helpers | Partially implemented | Source-linked summary categories postoje i helper semantika je izdvojena iz `core.py`. | Treba formalnije odvojiti official facts od future Finding objecta. |
