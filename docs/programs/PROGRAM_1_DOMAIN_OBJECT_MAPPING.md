...
| --- | --- | --- | --- | --- |
| Clinical Readiness Gate not fully implemented | `Clinical Readiness Gate`, `Clinical Readiness Status`, `Clinical Readiness Item` | B3 read-only preview plus B4 static demo/pilot templates, B5 metadata transparency, B6 demo explicit binding, B7 demo version metadata, B8 snapshot non-implementation metadata, B9 snapshot persistence design docs, B10 migration review, B11 capture endpoint design, B12 permission/audit contracts, B13 persistence table/model, B14 internal capture service, B15 backend capture endpoint, B16 read-only history endpoint, B17 read-only history UI, B18 reason-required capture UI, B19 read-only detail view, B20 duplicate-capture guard and B21 canonical disclaimer/immutability hardening | Confusion with `/api/readiness`; false clinical safety signal; premature blockers without override governance; static keyword/name/code config, demo version metadata and snapshot metadata could be mistaken for production rules; capture/history/detail UI could be mistaken for clinical approval if future labels are poor. | Snapshot Supersession Contract before supersession service/UI or enforcement |
