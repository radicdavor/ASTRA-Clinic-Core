...
| `Clinical Readiness Snapshot` | `ClinicalReadinessSnapshot`, `clinical_readiness_snapshots`, migration `0014_clinical_readiness_snapshots.py`, `clinical_readiness_snapshots.py` service, `POST /api/appointments/{appointment_id}/clinical-readiness-snapshots`, `GET /api/appointments/{appointment_id}/clinical-readiness-snapshots`, `AppointmentDetail` history section and reason modal | Backend capture/history plus frontend history/capture prototype | B13 dodaje DB shape za immutable copied preview payload, reason, disclaimer, template metadata i future supersession fields. B14 dodaje interni capture service s audit eventom. B15 dodaje permission-gated backend capture endpoint. B16 dodaje read-only, appointment-scoped, summary-only history endpoint. B17 dodaje Appointment Workspace read-only history surface. B18 dodaje reason-required capture modal koji osvjezava history. | Snapshot Detail Read-Only View prije supersession/edit/delete rasprave. |
...
| `AppointmentDetail` | Appointment workspace, materials, invoice, audit, read-only clinical readiness preview, snapshot history and reason-required snapshot capture modal. | `Appointment`, `Material Consumption`, `Inventory Movement`, `Clinical Readiness Preview`, `Clinical Readiness Snapshot` | Aligned with caution | Preview/history/capture nisu enforcement i ne smiju postati task, override, template editor, approval ili clearance surface. |
...
| --- | --- | --- | --- | --- |
| Clinical Readiness Gate not fully implemented | `Clinical Readiness Gate`, `Clinical Readiness Status`, `Clinical Readiness Item` | B3 read-only preview plus B4 static demo/pilot templates, B5 metadata transparency, B6 demo explicit binding, B7 demo version metadata, B8 snapshot non-implementation metadata, B9 snapshot persistence design docs, B10 migration review, B11 capture endpoint design, B12 permission/audit contracts, B13 persistence table/model, B14 internal capture service, B15 backend capture endpoint, B16 read-only history endpoint, B17 read-only history UI and B18 reason-required capture UI | Confusion with `/api/readiness`; false clinical safety signal; premature blockers without override governance; static keyword/name/code config, demo version metadata and snapshot metadata could be mistaken for production rules; capture/history UI could be mistaken for clinical approval if future labels are poor. | Snapshot Detail Read-Only View before supersession, editor or enforcement |
