...
| `Clinical Readiness Snapshot` | `ClinicalReadinessSnapshot`, `clinical_readiness_snapshots`, migration `0014_clinical_readiness_snapshots.py`, `clinical_readiness_snapshots.py` service, `POST /api/appointments/{appointment_id}/clinical-readiness-snapshots`, `GET /api/appointments/{appointment_id}/clinical-readiness-snapshots`, `GET /api/appointments/{appointment_id}/clinical-readiness-snapshots/{snapshot_id}`, `AppointmentDetail` history/detail section and reason modal | Backend capture/history/detail plus frontend history/capture/detail prototype | B13 dodaje DB shape za immutable copied preview payload, reason, disclaimer, template metadata i future supersession fields. B14 dodaje interni capture service s audit eventom. B15 dodaje permission-gated backend capture endpoint. B16 dodaje read-only, appointment-scoped, summary-only history endpoint. B17 dodaje Appointment Workspace read-only history surface. B18 dodaje reason-required capture modal koji osvjezava history. B19 dodaje read-only detail endpoint i panel za full copied payload. | Idempotency and Duplicate-Capture Guard prije supersession/edit/delete rasprave. |
...
| --- | --- | --- | --- | --- |
| Clinical Readiness Gate not fully implemented | `Clinical Readiness Gate`, `Clinical Readiness Status`, `Clinical Readiness Item` | B3 read-only preview plus B4 static demo/pilot templates, B5 metadata transparency, B6 demo explicit binding, B7 demo version metadata, B8 snapshot non-implementation metadata, B9 snapshot persistence design docs, B10 migration review, B11 capture endpoint design, B12 permission/audit contracts, B13 persistence table/model, B14 internal capture service, B15 backend capture endpoint, B16 read-only history endpoint, B17 read-only history UI, B18 reason-required capture UI and B19 read-only detail view | Confusion with `/api/readiness`; false clinical safety signal; premature blockers without override governance; static keyword/name/code config, demo version metadata and snapshot metadata could be mistaken for production rules; capture/history/detail UI could be mistaken for clinical approval if future labels are poor. | Snapshot Idempotency and Duplicate-Capture Guard before supersession, editor or enforcement |
