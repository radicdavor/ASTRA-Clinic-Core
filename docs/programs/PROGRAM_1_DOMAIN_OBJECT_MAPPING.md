...
| `Patient Clinical Knowledge` | `/patients/{id}/clinical-summary`, `PatientKnowledgeItem`, PatientDetail cards | Partially implemented | Source-linked summary categories postoje. | Treba formalnije odvojiti official facts od summary viewa. |
| `Patient Clinical Summary` | `PatientClinicalSummaryRecord`, summary endpoints, PatientDetail AI summary card | Implemented | Summary record/status postoji. | Ne smije se tretirati kao source of truth. |
| `Open Question` | `open_questions`, `open_items` | Partially implemented | Prikazuje nerazriješene stavke. | Nema lifecycle, owner, due date ili task conversion. |
| `Unresolved Finding` | `open_items`, recommendations pending text | Partially implemented | Izvedeno iz reviewed documents/placeholder extraction. | Nema formalni status model. |
...
| `Clinical Readiness Gate` | Program 1 docs | Future concept | Nema backend/frontend object. | Prototype tek nakon knowledge/document hardeninga. |
...
| --- | --- | --- | --- | --- |
| Clinical Readiness Gate not implemented | `Clinical Readiness Gate`, `Ready`, `Not Ready` | Documentation-only | Confusion with `/readiness`; false clinical safety signal. | Clinical Readiness Gate Prototype |
