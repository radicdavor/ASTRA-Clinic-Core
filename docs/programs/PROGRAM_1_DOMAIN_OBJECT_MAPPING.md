...
| `Clinical Readiness Snapshot` | `ClinicalReadinessSnapshot`, `clinical_readiness_snapshots`, migrations `0014`/`0015`, `clinical_readiness_snapshots.py` service, `POST /api/appointments/{appointment_id}/clinical-readiness-snapshots`, `GET /api/appointments/{appointment_id}/clinical-readiness-snapshots`, `GET /api/appointments/{appointment_id}/clinical-readiness-snapshots/{snapshot_id}`, `AppointmentDetail` history/detail section and reason modal | Backend capture/history/detail plus frontend history/capture/detail prototype | B13 dodaje DB shape za immutable copied preview payload, reason, disclaimer, template metadata i future supersession fields. B14 dodaje interni capture service s audit eventom. B15 dodaje permission-gated backend capture endpoint. B16 dodaje read-only, appointment-scoped, summary-only history endpoint. B17 dodaje Appointment Workspace read-only history surface. B18 dodaje reason-required capture modal koji osvjezava history. B19 dodaje read-only detail endpoint i panel za full copied payload. B20 dodaje idempotency persistence i duplicate-capture guard. B21 hardena canonical disclaimer i immutable payload discipline. B22 dokumentira future additive supersession contract. B23 dodaje interni supersession service i audit event bez endpointa/UI-ja. | Snapshot Supersession Endpoint Prototype prije UI implementacije. |
