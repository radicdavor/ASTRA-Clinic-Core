...
| HP01-004 | P2 | frontend/ux | Critical actions needed consistent Croatian action language, contextual help and confirmation behavior across patient, appointment, billing, procurement and API key workflows. | fixed-local | Codex | fixed | Addressed by V19 design system and `ActionButton`; does not expand medical scope. |
...
Run final validation after the V19 design-system fix. If checks pass, `v0.1-pilot` can move to an explicit maintainer tag decision.
